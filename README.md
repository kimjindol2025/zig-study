...
### Lesson 2-4 실행 (RESTful API)
```bash
zig build run-2-4
```

### Lesson 2-5 실행 (로깅 시스템 및 모니터링)
```bash
zig build run-2-5
```

### Lesson 2-6 실행 (보안 및 암호화 설계)
```bash
zig build run-2-6
```

### Lesson 3-1 실행 (베어 메탈 부트로더)
```bash
zig build run-3-1
```

### Lesson 3-2 실행 (GDT 및 IDT 설계)
```bash
zig build run-3-2
```

### Lesson 3-3 실행 (물리 메모리 관리자)
```bash
zig build run-3-3
```

### Lesson 3-4 실행 (가상 메모리 및 페이징)
```bash
zig build run-3-4
```
...
- **2-2**: 데이터베이스(SQL) 연동 및 인터페이스 설계 ✅ 💾 **저장소 계층!**
- **2-3**: 캐싱 전략(LRU Cache)과 동시성 제어 ✅ ⚡ **성능 극대화!**
- **2-4**: RESTful API 설계 및 JSON 직렬화 효율화 ✅ 🌐 **대외 창구!**
- **2-5**: 로깅 시스템 및 런타임 모니터링 설계 ✅ 📊 **관측성(Observability)!**
- **2-6**: 보안(Security) - 암호화와 인증 프로토콜 설계 ✅ 🛡️ **신뢰의 방패!**

### Zig 전공 301 (고급 - 운영체제)
- **3-1**: 베어 메탈(Bare Metal) 입문 - 부트 로더와 커널 진입 ✅ 🚀 **시스템 창조주!**
- **3-2**: GDT 및 IDT 설계 - CPU 보호 메커니즘과 인터럽트 처리 ✅ 🛡️ **메모리 보호!**
- **3-3**: 물리 메모리 관리자(PMM) 및 비트맵 설계 ✅ 💾 **메모리 지도!**
- **3-4**: 가상 메모리(Paging) 및 페이지 테이블 관리 ✅ 🌐 **환상의 마법!**

### Zig 전공 400+ (실무)
- **3-5**: 프로세스(Process)와 스레드(Thread) - 컨텍스트 스위칭 📅
- **3-6**: 제네릭 프로그래밍 (comptime) 📅
- **3-7**: 고성능 알고리즘 (SIMD, 벤치마킹) 📅
...
2026년 2월 26일 - **저장소 계층 설계 완성!** 💾 (Lesson 2-2: 데이터베이스 연동)
2026년 2월 26일 - **메모리 성능 극대화!** ⚡ (Lesson 2-3: 캐싱 전략)
2026년 2월 26일 - **대외 창구 설계 완성!** 🌐 (Lesson 2-4: RESTful API)
2026년 2월 26일 - **관측성 구축 완성!** 📊 (Lesson 2-5: 로깅 시스템 및 런타임 모니터링)
2026년 2월 26일 - **신뢰의 방패 완성!** 🛡️ (Lesson 2-6: 보안 및 암호화 설계) - **Zig 전공 201 완료!**
2026년 2월 26일 - **시스템 창조주 시작!** 🚀 (Lesson 3-1: 베어 메탈 부트로더) - **Zig 전공 301 시작!**
